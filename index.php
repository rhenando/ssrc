  ...
  <?php if(have_posts()): while(have_posts()): the_post(); ?>
    <article style="margin-bottom:36px;border:1px solid var(--border);border-radius:14px;overflow:hidden;">
      ...
      <div style="padding:26px 30px;">
        ...
        <?php the_excerpt(); ?>
        <a href="<?php the_permalink();?>" style="color:var(--teal);font-weight:600;font-size:.88rem;display:inline-flex;align-items:center;gap:4px;margin-top:10px;">Read More →</a>
      </div>
    </article>
  <?php endwhile; else: ?>
    <p style="color:var(--muted);text-align:center;padding:60px 0;">No posts found.</p>
  <?php endif; ?>
</div>
<?php get_footer(); ?>
