<?php
// Redirect to Arabic footer if needed (Elementor Compatibility)
if ( function_exists('ssrc_is_arabic') && ssrc_is_arabic() ) {
    get_template_part('footer', 'ar');
    return;
}
?>
 </main>
  ...
  <footer id="colophon" class="bg-navy-dark border-t-[3px] border-teal font-body">
    ...
    <div class="max-w-[1200px] mx-auto px-6 md:px-10 pt-14 pb-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
      ...
      <div>
        <?php if ( is_active_sidebar('footer-1') ) :
          ...
          <span class="block text-white text-[.78rem] font-bold uppercase tracking-[1.3px] pb-2.5 mb-4 border-b-2 border-teal w-fit">
            <?php esc_html_e('About Us','ssrc'); ?>
          </span>
          <ul class="flex flex-col gap-2">
            <?php foreach (['Our Mission'=>'/about','Our Team'=>'/about','Our History'=>'/about','Careers'=>'/careers'] as $label => $path) : ?>
              <li>
                <a href="<?php echo esc_url(ssrc_lang_url($path)); ?>"
                   class="text-white/55 text-sm block py-0.5 hover:text-white/90 hover:pl-1.5 transition-all duration-150">
      ...
      <div>
        <?php if ( is_active_sidebar('footer-2') ) :
          ...
          <span class="block text-white text-[.78rem] font-bold uppercase tracking-[1.3px] pb-2.5 mb-4 border-b-2 border-teal w-fit">
            <?php esc_html_e('Quick Links','ssrc'); ?>
          </span>
          <ul class="flex flex-col gap-2">
            <?php foreach (['Home'=>'/','About'=>'/about','Services'=>'/services','Downloads'=>'/downloads'] as $label => $path) : ?>
              <li>
                <a href="<?php echo esc_url(ssrc_lang_url($path)); ?>"
                   class="text-white/55 text-sm block py-0.5 hover:text-white/90 hover:pl-1.5 transition-all duration-150">
                  <?php echo esc_html($label); ?>
      ...
      <div>
        <?php if ( is_active_sidebar('footer-3') ) :
          ...
          <span class="block text-white text-[.78rem] font-bold uppercase tracking-[1.3px] pb-2.5 mb-4 border-b-2 border-teal w-fit">
            <?php esc_html_e('Our Services','ssrc'); ?>
          </span>
          <ul class="flex flex-col gap-2">
            <?php foreach (['Market Research','Data Analytics','Policy Studies'] as $label) : ?>
              <li>
      ...
      <div>
        ...
        ?>
          <span class="block text-white text-[.78rem] font-bold uppercase tracking-[1.3px] pb-2.5 mb-4 border-b-2 border-teal w-fit">
            <?php esc_html_e('Contact Us','ssrc'); ?>
          </span>
    ...
    <div class="max-w-[1200px] mx-auto px-6 md:px-10 py-4 border-t border-white/10 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex gap-2">
        <?php foreach ([['Facebook','f'],['Twitter','𝕏'],['LinkedIn','in']] as $s) : ?>
      ...
      <p class="text-[.8rem] text-white/35">
        <?php echo wp_kses_post(get_theme_mod('ssrc_copyright','&copy; ' . date('Y') . ' SSRC. All rights reserved.')); ?>
      </p>
